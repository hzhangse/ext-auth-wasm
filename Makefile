PLUGIN_NAME ?= ext-auth-wasm
BUILDER_REGISTRY ?= higress-registry.cn-hangzhou.cr.aliyuncs.com/plugins/
#REGISTRY ?=test:5000/plugins/
REGISTRY ?=registry.cn-shanghai.aliyuncs.com/rainbow954/
GO_VERSION ?= 1.24.4
...
local-run:
	envoy -c ./config.yaml --concurrency 0 --log-level debug --component-log-level wasm:debug
